...
openssl req -newkey rsa:2048 -nodes -keyout $outdir/tls.key -subj "/C=US/ST=CA/L=Los Angeles/O=Hello World/CN=*.${INSTANA_BASE_DOMAIN}" -out $outdir/$core_tls_csr
...
openssl x509 -req -extfile <(printf "subjectAltName=DNS:${INSTANA_BASE_DOMAIN},DNS:${INSTANA_TENANT_DOMAIN},DNS:${INSTANA_AGENT_ACCEPTOR}") \
  -days 365 -in $outdir/$core_tls_csr -CA $outdir/ca.crt -CAkey $outdir/ca.key -CAcreateserial -out $outdir/$core_tls_cert
